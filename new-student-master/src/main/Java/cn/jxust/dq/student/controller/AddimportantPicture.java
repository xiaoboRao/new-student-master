 ...
 */
@Controller
public class AddimportantPicture {
    @Autowired
    ...
    @RequestMapping(value = "/addimportantpic")
    public String addimportantpic(MultipartFile importantpic, Model model, HttpServletRequest request) throws IOException {
        String filestr=importantpic.getOriginalFilename();
        ...
        //将图片保存在虚拟路径中
        if (filename.trim() != "") {
            importantpic.transferTo(new File("C:\\Users\\Administrator\\Desktop\\new-student-master\\new-student-master\\src\\main\\webapp\\picture\\" + filename));
            //将路径保存在数据库中
            //获得用户id，并保存到图片路径中；
            myimportant.setMyimportantpic(filename);
            ...
            if(items!=null) {
                ...
                myimportant.setItemsId(item.getId());
                //将mygroup插入到数据库
                myimportantservice.insert(myimportant);
                //要将图片取出来放入到数组中然后组装pricinpal
                //首先根据itemid获得mygroup
                List<Myimportant> mysocialList = myimportantservice.findbyitemsid(item.getId());
                if(mysocialList!=null) {
                    for (int i = 0; i <mysocialList.size(); i++) {
                        importantpics[i]=mysocialList.get(i).getMyimportantpic();
                    }
                }
            }
