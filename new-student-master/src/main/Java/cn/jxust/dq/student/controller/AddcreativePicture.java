 ...
 */
@Controller
public class AddcreativePicture {
    @Autowired
    ...
    @RequestMapping(value = "/addmycreativepic")
    public String addimportantpic(MultipartFile creativepic, Model model, HttpServletRequest request) throws IOException {
        String filestr=creativepic.getOriginalFilename();
        ...
        //将图片保存在虚拟路径中
        if (filename.trim() != "") {
            creativepic.transferTo(new File("C:\\Users\\Administrator\\Desktop\\new-student-master\\new-student-master\\src\\main\\webapp\\picture\\" + filename));
            //将路径保存在数据库中
            //获得用户id，并保存到图片路径中；
            mycreative.setMycreativepic(filename);
            ...
            if(items!=null) {
                ...
                mycreative.setItemsId(item.getId());
                //将mygroup插入到数据库
                mycreativeservice.insert(mycreative);
